...
class NERModel(LanguageModel):
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer", initializer=xavier_weight_init()):
        ...
        z =tf.nn.tanh(tf.matmul(window, W) + b1)
        #tf.add_to_collections(names, value) : Stores value in the collections given by names
    ...
    with tf.variable_scope("Softmax", initializer=xavier_weight_init()):
        ...
        b2=tf.get_variable('b1', [self.config.label_size])
        y =tf.matmul(z, U) + b2
