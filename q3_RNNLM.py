...
class RNNLM_Model(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    labels_placeholder: Labels placeholder tensor of shape
                        (None, num_steps), type tf.float32 !!!WARN=>int32!!!
  ...
  def add_embedding(self):
    ...
    with tf.device('/gpu:0'):
      ...
      #inputs = [tf.squeeze(i) for i in tf.split(1, self.config.num_steps, embed)]
      ###unpack a Tensor of shape [n, m, o] as the sequence, into a sequence of n-tensors, each with shape [m, o].
      ###http://r2rt.com/recurrent-neural-networks-in-tensorflow-ii.html
      inputs = tf.unpack(embed, axis=1)
      ### END YOUR CODE
      return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("Layer"):
        ...
        outputs = [tf.matmul(out, U) + b_2 for out in rnn_outputs]
    ### END YOUR CODE
    return outputs
  ...
  def add_loss_op(self, output):
    ...
    reshaped_labels = tf.reshape(self.labels_placeholder, [self.config.batch_size*self.config.num_steps, -1])
    cross_entropy = sequence_loss([output],[reshaped_labels], loss_init, len(self.vocab))
  ...
  def add_model(self, inputs):
    ...
    #and gets updated after reading each word (https://www.tensorflow.org/tutorials/recurrent/)
    self.initial_state = tf.ones([self.config.batch_size, self.config.hidden_size])
    ...
    for input in inputs:
        input_dropout = tf.nn.dropout(input, self.dropout_placeholder)
        state = tf.sigmoid(tf.matmul(state, H) + tf.matmul(input_dropout, I) + b_1)
        output_dropout = tf.nn.dropout(state, self.dropout_placeholder)
        rnn_outputs.append(output_dropout)
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in range(stop_length):
    ### YOUR CODE HERE
    generate_feed = {model.input_placeholder: [tokens[-1:]],
                     model.initial_state: state,
                     model.dropout_placeholder: 1}

    state, y_pred = session.run(
        [model.final_state, model.predictions[-1]], feed_dict=generate_feed)
    ### END YOUR CODE
